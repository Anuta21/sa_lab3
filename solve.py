...
def legendre_polynomial(degree, x):
    if degree == 0:
        ...
        return ((2 * degree - 1) * x * legendre_polynomial(degree - 1, x) - (degree - 1) * legendre_polynomial(degree - 2, x)) / degree
...
class Solve(object):
    ...
    def __init__(self, d):
        ...
        self.filename_output = 'output.xlsx'
    ...
    def define_data(self):
        ...
        # print('!!!!')
        # print(len(np.fromstring(self.filename_input, sep='\t')))
        # print(np.fromstring(self.filename_input, sep='\t'))
    ...
    def poly_func(self):
        ...
        """
        # change
        if self.poly_type == 'Поліноми Лежандра':
            self.poly_f = lambda deg, x: special.eval_legendre(deg, x) 
        elif self.poly_type == 'Зсунуті поліноми Лежандра':
            self.poly_f = lambda deg, x: special.eval_sh_legendre(deg, x) / (deg + 1)
        elif self.poly_type == 'Синус':
